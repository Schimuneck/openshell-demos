...
> **Parallel operation:** This demo runs alongside the OpenClaw demo on the same OpenShell gateway. See [`../openclaw/`](../openclaw/) for the companion demo.
...
cd ~/projects/redhat/openshell-demos/opencode
```

6. Set your Jira API token (so the sandbox create command can reference it):

```fish
set -x JIRA_API_TOKEN "ATATT3xFfGF0TJeoRgM217uA8yI5cBR3RDQWm2zxZ7F6jKji57nru2A4uxbcxLXo9Ri0TnMFhd5gRJIx_mIezfdipr7fmlX0-Vmr4u2SmDPDq8qFBrQGTXfDqZQcJx1I-Sx6weCh_LKr_5z9aZ8Q54qnrnstPN1ERlOG9AUzX-RkbFbl_f0FMuw=D7738646"
```

7. Populate `.env` with your credentials if using the script (gitignored, never committed):
...
8. Build the MLflow plugin bundle (if not already done):
...
Show the customer what's running on the cluster:

```bash
oc -n openshell get pods
```

This shows the OpenShell gateway pod, the PostgreSQL database pod, and the sandbox pod you just created. Point out that the sandbox is a regular Kubernetes pod — OpenShell works with existing OpenShift infrastructure.
...
Point out two things in the policy:

1. The `binaries` field: only the OpenCode binary gets network access. Not curl, not wget, not any other process.
2. Two Google endpoints: `aiplatform.googleapis.com` (Vertex AI inference) and `oauth2.googleapis.com` (ADC token exchange). The sandbox uses Google's Application Default Credentials — a local credential file gets exchanged for short-lived access tokens automatically. The agent never handles raw API keys.
